            ...
            content:
              [
                ...
                },
                {
                  ...
                  text: [
                    "1.行情显示图: 模拟了期货的行情曲线图",
                    "2.价格信息表: 展示了期货在当次的最高最低价、实时价格等，是最重要的信息之一。",
                    "3.风控仪: 用数字来体现当前操作的风险度，数字越高则越需谨慎操作。",
                    "4.期货选择键: 本游戏一共有2种期货可供选择，对不同的期货进行操作时，采用的策略也应改变。",
                    "5.时间控制栏: 时间控制栏目中的一段时间（例如10秒）代表了现实中的一天时间，随着时间的推移，期货价格发生波动，系统也会定时进行准备金结算。“调节时间速度键”则用来调整游戏时间和现实时间的换算比例。",
                    "6.提示信息栏: 实时显示用户进行的操作，并且当特殊情况发生时显示提示信息。",
                    "6.用户资金信息栏: 包含用户资金、盈亏额、保证金等资金信息。",
                  ]
                }
              ]
          },
              ...
              [
                {
                  ...
                  text:[
                    ...
                    "3.开始游戏: 点击“开始”按钮进行一局游戏，当下方的时间变为0时游戏终止，系统将为您计算盈亏额并提供改善建议。",
                    "4.下单: 游戏开始后，您可以在任何时候进行下单（当然，切记真实的期货市场有交易时间的限制），下单分为4种，即买入开仓、卖出开仓、买入平仓、卖出平仓。",
                    "5.结算: 每隔固定时间（即现实中的一天），系统将对每一个会员、客户的盈亏、交易手续费、交易保证金等款项进行结算。如果您的保证金过低，系统将提醒您追加保证金。",
                    "6.提示: 每当进行交易、结算或发生保证金不足等情况，提示信息栏中将会产生记录。",
                    "6.结束: 如果您觉得自己赚的够多了，点击“结束”按钮立即结束游戏，当计时为0时，您也不得不结束该轮游戏。",
                  ]
                }
              ]
          },
